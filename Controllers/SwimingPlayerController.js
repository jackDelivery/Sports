...
const CreateSwimingPlayer = asyncHandler(async (req, res) => {
    try {
        ...
        const user2 = await cricketPlayerModel.findOne({ email });
...
const loginAdmin = asyncHandler(async (req, res) => {
    ...
    const findAdmin = await swimingPlayerModel.findOne({ email });
...
const AdminApproved = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        const approved = await swimingPlayerModel.findByIdAndUpdate(_id, {
...
const adminApprovedGet = asyncHandler(async (req, res) => {
    try {
        ...
        let result = await swimingPlayerModel.find({});
...
const AdminDeleteUsers = asyncHandler(async (req, res) => {
    ...
    try {
        const deletedUser = await swimingPlayerModel.findByIdAndDelete(id); // Using id parameter to find and delete user
...
const users = asyncHandler(async (req, res) => {
    try {
        ...
        const users = await swimingPlayerModel.find({});
...
module.exports = { CreateSwimingPlayer, login, createNicPdf, loginAdmin, AdminApproved, AdminDeleteUsers, users, adminApprovedGet }