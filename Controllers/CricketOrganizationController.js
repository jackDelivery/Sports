...
const createOrganizationPlayer = asyncHandler(async (req, res) => {
    try {
        ...
        const user = await cricketOrganizationModel.findOne({ email });

        const user2 = await swimmingOrganizationModel.findOne({ email });
        ...
        const User = new cricketOrganizationModel({
            name: name,
            email: email,
            password: password,
            number: number,
